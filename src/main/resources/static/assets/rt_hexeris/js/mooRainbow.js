...
var MooRainbow = new Class({
	...
	wheelEvents: function() {
		var arrColors = Array.from(this.arrRGB).extend(this.arrHSB);
	...
	doLayout: function() {
		var id = this.options.id, prefix = this.options.prefix, self = this;
		...
		if (this.options.transparent) {
			var transp = new Element('div', {
				'class': prefix + 'transpButton'
			}).inject(box).set('html', '<span>X</span>').addEvent('click', function() {
				self.fireEvent('onChange', 'transparent');
				ok.fireEvent('click');
			});
		}

		if (!window.opera) this.rePosition();

		var overlays = document.getElements('#' + idPrefix + 'overlay');
